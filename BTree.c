...
int deleteNode(btNode* x, int k, bTree* tree) {
	...
	if (x->keys[i] == k) {
		// Case 2-b-1 : (i)th child node has key_num larger than t-1
		...
		// Case 2-b-2 : (i+1)th child node has ken_num larger than t-1
		else if (x->child_pointer[i+1]->key_num > t - 1) {
			int replace_num = find_succ(x->child_pointer[i + 1]);
			x->keys[i] = replace_num;//x->keys[i] = K
			deleteNode(x->child_pointer[i + 1], replace_num, tree);
		}
		// Case 2-a : Merge (i)th child node with (i+1)th child node if both node's key_num is equal to t-1
		else {
			mergeRightNode(x, i);
